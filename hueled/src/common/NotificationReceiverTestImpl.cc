...
extern void send_fwUpgradeMsg(char *url, char *deviceId);
...
void NotificationReceiverTestImpl::Receive(Notification const& notification) {
    ...
    if ((m_Applications.size() == 0) || (find(m_Applications.begin(), m_Applications.end(), appName) !=  m_Applications.end())) {
        ...
        for (std::vector<NotificationText>::const_iterator vecMessage_it = vecMessages.begin(); vecMessage_it != vecMessages.end(); ++vecMessage_it) {
          ...
          // send_fwUpgradeMsg("http://172.17.60.162/fw/dev_opkg.bin");
               if (!vecMessage_it->getText().c_str())
            {
                 std::cout << "Failed to get notification!" << std::endl;
                 return;
            } 
            
              //  client_t *c1;
              //  c1->fd_ = g_listenFd;
                std::cout << "Get vecMessage succeed!"<< std::endl;
                cJSON *json, *json_url, *json_deviceId;
                //char *p[MAX_TEXT_LEN] = vecMessage_it->getText().c_str();
                json = cJSON_Parse(vecMessage_it->getText().c_str());
                std::cout << "Parse vecMessage succeed!"<< std::endl;


                if (!json)
                {
                    std::cout << "not json format!" << std::endl;
                    std::cout << "Error befor:" << cJSON_GetErrorPtr() << std::endl;
                    return;
                }
                json_deviceId = cJSON_GetObjectItem(json, "deviceId");
                json_url = cJSON_GetObjectItem(json, "IpmPath");
                if (json_deviceId == NULL || json_url == NULL)
                {
                    std::cout << "the deviceId or the URL is NULL!" << std::endl;
                    return;
                }
                if (json_deviceId->type != cJSON_String || json_url->type != cJSON_String)
                {
                    std::cout << "param type is wrong !" << std::endl;
                    return;
                }
                char *u = json_url->valuestring;
                char *d = json_deviceId->valuestring;
                std::cout << "Get URL succeed!"<< u <<std::endl;
                std::cout << "Get deviceId succeed!"<< d << std::endl;

                
                send_fwUpgradeMsg(u,d);
               
                //handle_firmware_upgrade(c1, json_url->valuestring);   
                std::cout << "One loop finish! to join another loop!" << std::endl;       
        }
