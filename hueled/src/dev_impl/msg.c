...
int devMsg_initServerSocket(unsigned int port)
{
    ...
    {
        adapt_error("Could not create socket");
        return -1;
    }
    ...
    {
        adapt_error("setsockopt failed! %s\r\n", strerror(errno));
        return -1;
    }
    ...
    if (rc != 0)
    {
        adapt_error("bind to port %d failed, rc=%d errno=%d\r\n", port, rc, errno);
        close(fd);
        return -1;
    }
    ...
    if (rc != 0)
    {
        adapt_error("listen to port %s failed, rc=%d errno=%d\r\n", port, rc, errno);
        close(fd);
        return -1;
    }

    adapt_debug("server socket opened and ready (fd=%d)\r\n", fd);
...
int devMsg_receive(int fd, DevMsgHeader **buf)
{
    ...
    if (msg == NULL)
    {
        adapt_error("alloc of msg header failed\r\n");
        return -2;
    }
    ...
    {
        adapt_error("bad read, rc=%d errno=%d\r\n", rc, errno);
        free(msg);
        return -1;
    }
    ...
    if (msg->dataLength > 0)
    {
        if (msg->dataLength > MAX_DEV_MSG_LEN)
        {
            ...
            if (msg == NULL)
            {
                adapt_error("realloc to %d bytes failed\r\n", sizeof(DevMsgHeader) + msg->dataLength);
                free(msg);
                return -2;
            }
        }
        ...
        if (rc <= 0)
        {
            adapt_error("bad data read, rc=%d errno=%d\r\n", rc, errno);
            free(msg);
            return -1;
        }
        else if (rc < msg->dataLength) {
            adapt_error("bad data read, rc=%d expected=%d\r\n", rc, msg->dataLength);
            free(msg);
            return -1;
        }
    }
...
int devMsg_send(client_t *c, DevMsgHeader *buf)
{
   ...
   if (rc < 0)
   {
      if (errno == EPIPE)
      {
         adapt_debug("got EPIPE, dest app is dead\r\n");
         return -2;
      }
      else
      {
         adapt_debug("write failed, errno=%d\r\n", errno);
         ret = -1;
      }
   }
   else if (rc != (int) totalLen)
   {
      adapt_debug("unexpected rc %d, expected %u", rc, totalLen);
      ret = -1;
   }
