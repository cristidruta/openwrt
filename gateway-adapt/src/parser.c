...
int updateConfigInfo(char *updateBuf, char *deviceType, char *manufacture, char *moduleNumber, char *devSn, char *configName, char *configValue)
{
    ...
    if(fp == NULL)
    {
        ...
        adapt_error("open %s error!\n", DATA_CONVERT_FILE_PATH);
        return -1;
    }
    else
    {
        //printf("open %s succeed!\n", DATA_CONVERT_FILE_PATH);
        adapt_error("open %s succeed!\n", DATA_CONVERT_FILE_PATH);
    }
    ...
    if(tree == NULL)
    {
        ...
        adapt_error("Load file error!\n");
        return -1;
    }
    else
    {
        //printf("Load file succeed!\n");
        adapt_debug("Load file succeed!\n");
    }
    ...
    if(node == NULL)
    {
        ...
        adapt_error("can not find element node!\n");
        mxmlDelete(tree);
        return -1;
    }
    ...
    if(deviceTypeNode == NULL)
    {
        ...
        adapt_error("cannot find type node %s\n", deviceType);
        mxmlDelete(tree);
        return -1;
    }
    else
    ...
    if(manufacture == NULL)
    {
        ...
        adapt_error("cannot find manufacture node %s\n", manufacture);
        mxmlDelete(tree);
        return -1;
    }
    else
    {
        ...
        adapt_debug("find manufacture node %s\n", manufacture);
    }
    ...
    if(configKeyNode == NULL)
    {
        ...
        adapt_error("cannot find moduleNumber node %s\n", moduleNumber);
        mxmlDelete(tree);
        return -1;
    }
    else
    {
        ...
        adapt_debug("find moduleNumber node %s\n", moduleNumber);
    }
            ...
            node_config = mxmlFindElement(node_config, configKeyNode, "configKey", NULL, NULL, MXML_DESCEND))
    {
        ...
        if(strcmp(mxmlElementGetAttr(node_config,"name"), "devData") == 0)
        {
                    ...
                    node_detail = mxmlFindElement(node_detail, node_config, "detail", NULL, NULL, MXML_DESCEND))
            {
                ...
                adapt_debug("node_detail = %p\n", node_detail);
                //printf("detail txt: %s\n", mxmlElementGetAttr(node_detail,"name"));
                adapt_debug("detail txt: %s\n", mxmlElementGetAttr(node_detail,"name"));
                ...
                if(json_keyNode == NULL)
                {
                    ...
                    adapt_debug("cannot find %s key in devData\n", mxmlElementGetAttr(node_detail,"name"));
                }
                else
                {
                    ...
                    adapt_debug("find %s key in devData\n", configName);

                    //printf("configName = %s, configValue = %s\n", configName, configValue);
                    adapt_debug("configName = %s, configValue = %s\n", configName, configValue);
        ...
        else
        {
            ...
            adapt_debug("no need to handle this config\n");
        }
    }
