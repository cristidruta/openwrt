...
static int verify_callback(int ok, X509_STORE_CTX *store __attribute__((unused)))
{
   ...
   if (ok == 0)
   {
      cmsLog_error("error_num = %d, err_msg = %s, depth = %d,\nsubject = %s,\nissuer = %s\n", 
                   err, X509_verify_cert_error_string(err), depth, subject, issuer);
   }
   else
   {
      cmsLog_debug("error_num = %d, err_msg = %s, depth = %d,\nsubject = %s,\nissuer = %s\n", 
                   err, X509_verify_cert_error_string(err), depth, subject, issuer);
   }
      ...
      if ((pCN = strstr(subject, "CN=")) != NULL)
      {
         ...
         if ((pEnd = strchr(pCN, '/')) != NULL)
         {
            *pEnd = '\0';
            ...
            {
               cmsLog_debug("return X509_V_OK, CN = %s, URL = %s\n", pCN, acsState.acsURL);
            }
            else
            {
               ok = 0;
               ...
               cmsLog_error("return X509_V_ERR_APPLICATION_VERIFICATION, CN is not in URL, CN = %s, URL = %s\n", pCN, acsState.acsURL);
            }
         }
      }
   }
...
void proto_Init()
{
   ...
   {
      cmsLog_error("Could not set cipher list for SSL");
   }
#endif // ORIGINAL_OPENSSL
         ...
         if (errno == EAGAIN)
         {
            ...
            else
            {
               ...
               else
               {
                   ...
                   * and set acsState.fault to CMSRET_INTERNAL_ERROR
                   */
                  acsState.fault = fault = CMSRET_INTERNAL_ERROR;
...
static void addCookieHdr( CookieHdr **p, char *c)
{
   ...
   memset(newCookie, 0, sizeof(CookieHdr));
