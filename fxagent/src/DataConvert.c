...
int db_connect()
{
    ...
    if(ret != SQLITE_OK)
    {
        cloudc_error("open %s failed! %s", DEVICE_DATABASE_PATH, sqlite3_errmsg(db));
        sqlite3_free(errmsg);
        return ret;
    }
    cloudc_debug("Open sqlite3 database [device.db] successfully!");
#if 0
    ret = sqlite3_open(":memory:", &mdb);
    if (ret != SQLITE_OK) {
        cloudc_error("open memory database error[%s]", sqlite3_errmsg(mdb));
        return 1;
    }
...
void getObjInfoByDevId(char *deviceId, char *interfaceName, char *objectPath, char *onlineStatus)
{
...
#if 0
    //intf=feixun.Flight.bc53e808004b1200.Config objPath=/bc53e808004b1200/Config status=on
    strncpy(interfaceName, "feixun.Flight.bc53e808004b1200.Config", MAX_INTERFACE_LEN - 1);

    strncpy(objectPath, "/bc53e808004b1200/Config", MAX_OBJECTPATH_LEN - 1);

    strncpy(onlineStatus, "on", MAX_ONLINESTATUS_LEN - 1);
    ...
    if(ret == SQLITE_OK)
    {
        ...
        for(i = 0; i < nrow; i++)
        {
            ...
            interfaceName[MAX_INTERFACE_LEN-1] = '\0';

            strncpy(objectPath, dbresult[index+1], MAX_OBJECTPATH_LEN - 1);
            objectPath[MAX_OBJECTPATH_LEN-1] = '\0';

            strncpy(onlineStatus, dbresult[index+2], MAX_ONLINESTATUS_LEN - 1);
            onlineStatus[MAX_ONLINESTATUS_LEN-1] = '\0';

            index+=ncolumn;

            cloudc_debug("row[%d]: intf=%s objPath=%s status=%s", i, interfaceName, objectPath, onlineStatus);
        }
    }
    else
    {
        cloudc_debug("query failed! %s ", sqlite3_errmsg(db));
        sqlite3_free(errmsg);
    }
...
int setDevIdByDevSn(char *deviceId, char *deviceSn)
{
    ...
    {   
        ...
        if (ret != SQLITE_OK) { 
            cloudc_error("update table deviceinfo Error: %s\n", sqlite3_errmsg(db));
            sqlite3_free(errmsg);
            ret = -1; 
        }
        else
...
int getAlljoynKeyName(char *deviceType, char *manufacture, char *moduleNumber, struct ipk_info *platformKeyName, int keyNameNumber)
{
    ...
    cloudc_debug("%s[%d]: Enter", __func__, __LINE__);
    cloudc_debug("%s[%d]: deviceType = %s, manufacture = %s, platformKeyNameHead = %p", 
                __func__, __LINE__, deviceType, manufacture, platformKeyName);
    ...
    if(platformKeyName != NULL)
    {
        ...
        {
            snprintf(platformKeyNameNodePath, KEY_NAME_PATH_MAX_LEN, "node/%s/%s/%s/%s", 
                    deviceType, manufacture, moduleNumber, platformKeyName->op_ipk_name);
