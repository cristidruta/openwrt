...
int devOnlineHandle()
{
    ...
    int devStatus=-1;
    char *manufacture="tuya";
    char *devSn="002000345ccf7f1b7e15";
    char *devType="Led";
    char *moduleNumber="led";
    char *devData = "{\"name\":\"Smart Builb-7e15\", \
\"power\":\"1\", \
...
\"softwareVersion\":\"V3.0\"}";
    ...
    devStatus=getDevStsByDevSn(manufacture, devSn);
    //if ( 0 == devStatus || -1 == devStatus )
    {
        ...
        snprintf(factoryConfigFileName, sizeof(factoryConfigFileName), 
                "/var/Factory%s%s%s.conf", manufacture, moduleNumber, devSn);
        snprintf(configFileName, sizeof(configFileName), "/var/%s%s%s.conf", manufacture, moduleNumber, devSn);
        ...
        snprintf(intfName, sizeof(intfName), "%s.%s.%s.Config", manufacture, moduleNumber, devSn);
        snprintf(objectPath, sizeof(objectPath), "/%s/Config", devSn);
