...
class AboutData : public AboutDataListener {
  public:
    ...
    QStatus SetPowerSwitch(const char* powerSwitch);

    QStatus GetPowerSwitch(char** powerSwitch);

    QStatus SetLightBulbColor(const char*lightBlubColor);

    QStatus GetLightBulbColor(char** lightBlubColor);
  ...
  public:
    ...
    static const char* POWER_SWITCH; ///< The power switch for led
    static const char* LIGHTBULB_COLOR; ///< The light bulb color for led
    // @}
  protected:
    /**
