...
const char* AboutData::POWER_SWITCH= "PowerSwitch";
const char* AboutData::LIGHTBULB_COLOR= "LightBulbColor";
...
void AboutData::InitializeFieldDetails() {
    ...
    aboutDataInternal->aboutFields[POWER_SWITCH] = FieldDetails(REQUIRED | ANNOUNCED | LOCALIZED, "s");
    aboutDataInternal->aboutFields[LIGHTBULB_COLOR] = FieldDetails(REQUIRED | ANNOUNCED | LOCALIZED, "s");
}
        ...
        if (IsFieldRequired(it->first.c_str())) {
            if (IsFieldLocalized(it->first.c_str())) {
                if (aboutDataInternal->localizedPropertyStore.find(it->first) == aboutDataInternal->localizedPropertyStore.end()) {
                    std::cout << "name: " << it->first.c_str() << "localized" << std::endl;
                    return false;
                } else {
                    if (language == NULL) {
                        ...
                        if (status != ER_OK) {
                            std::cout << "name: " << it->first.c_str() << "defaultLanguage" << std::endl;
                            return false;
                        }
                        if (aboutDataInternal->localizedPropertyStore[it->first].find(defaultLanguage) == aboutDataInternal->localizedPropertyStore[it->first].end()) {
                            std::cout << "name: " << it->first.c_str() << "localized defaultLanguage end" << std::endl;
                            return false;
                        }
                    } else {
                        if (aboutDataInternal->localizedPropertyStore[it->first].find(language) == aboutDataInternal->localizedPropertyStore[it->first].end()) {
                            std::cout << "name: " << it->first.c_str() << "localized Language end" << std::endl;
                            return false;
                        }
                    }
                }
                ...
                if (aboutDataInternal->propertyStore.find(it->first) == aboutDataInternal->propertyStore.end()) {
                    std::cout << "name: " << it->first.c_str() << "not localized Language end" << std::endl;
                    return false;
                }
            }
        }
...
QStatus AboutData::SetPowerSwitch(const char* powerSwitch)
{
    QStatus status = ER_OK;
    MsgArg arg;
    status = arg.Set(aboutDataInternal->aboutFields[POWER_SWITCH].signature.c_str(), powerSwitch);
    if (status != ER_OK) {
        return status;
    }
    status = SetField(POWER_SWITCH, arg);
    return status;

}

QStatus AboutData::GetPowerSwitch(char** powerSwitch)
{
    QStatus status;
    MsgArg* arg;
    status = GetField(POWER_SWITCH, arg);
    if (status != ER_OK) {
        return status;
    }
    status = arg->Get(aboutDataInternal->aboutFields[POWER_SWITCH].signature.c_str(), powerSwitch);
    return status;
}

QStatus AboutData::SetLightBulbColor(const char* lightBlubColor)
{
    QStatus status = ER_OK;
    MsgArg arg;
    status = arg.Set(aboutDataInternal->aboutFields[LIGHTBULB_COLOR].signature.c_str(), lightBlubColor);
    if (status != ER_OK) {
        return status;
    }
    status = SetField(LIGHTBULB_COLOR, arg);
    return status;
}
   
QStatus AboutData::GetLightBulbColor(char** lightBlubColor)
{
    QStatus status;
    MsgArg* arg;
    status = GetField(LIGHTBULB_COLOR, arg);
    if (status != ER_OK) {
        return status;
    }
    status = arg->Get(aboutDataInternal->aboutFields[LIGHTBULB_COLOR].signature.c_str(), lightBlubColor);
    return status;
} 
