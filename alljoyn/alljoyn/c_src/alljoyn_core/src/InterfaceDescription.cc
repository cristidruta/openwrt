...
QStatus InterfaceDescription::SendCommandToDev(SocketFd socketFd, char *name, char *value, struct sockaddr *addr) const
{
    const int SEND_BUF_LEN = 512;
    char send_buf[SEND_BUF_LEN];
    int ret = 0;

    sprintf(send_buf, "Send %s:%s", name, value);
    ret = sendto(socketFd, send_buf, strlen(send_buf), 0, (struct sockaddr *)addr, sizeof(struct sockaddr));
    if (ret == -1) {
        return ER_FAIL;
    } else {
        return ER_OK;
...
QStatus InterfaceDescription::GetDescriptionSocket(int *fd) const
{
...
QStatus InterfaceDescription::GetDescriptionClientAddr(struct sockaddr *clientAddr) const
{
    memcpy(clientAddr, &(this->devAddr), sizeof(struct sockaddr));

    return ER_OK;
}

QStatus InterfaceDescription::SetDescriptionSocket(int fd)
{
...
QStatus InterfaceDescription::SetDescriptionClientAddr(struct sockaddr *clientAddr)
{
    printf("enter SetDescriptionClientAddr");
    printf("devAddr %p", &(devAddr));
    memcpy(&(this->devAddr), clientAddr, sizeof(struct sockaddr));

    return ER_OK;
}

QStatus InterfaceDescription::SetDescriptionDevType(int devType)
{
    std::cout << "set device Type:" << devType << std::endl;
...
QStatus InterfaceDescription::GetDescriptionDevType(int *devType) const
{
