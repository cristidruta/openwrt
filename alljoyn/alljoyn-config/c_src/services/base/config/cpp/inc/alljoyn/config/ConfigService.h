...
class ConfigService : public ajn::BusObject {
  public:
     ...
     */
    ConfigService(ajn::BusAttachment& bus, AboutDataStoreInterface& store, Listener& listener, const char *path);
     ...
     * @param listener reference
     */
    ConfigService(ajn::BusAttachment& bus, PropertyStore& store, Listener& listener, const char *path);
    /**
     ...
     * @param ifcName alljoyn interface name
     * @return status.
     */
    QStatus Register(const char *ifcName);
