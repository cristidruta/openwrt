...
ConfigService::ConfigService(ajn::BusAttachment& bus, AboutDataStoreInterface& store, Listener& listener, const char *path) :
    //BusObject("/Config"), m_BusAttachment(&bus), m_AboutDataStore(&store), m_PropertyStore(NULL), m_Listener(&listener)
    BusObject(path), m_BusAttachment(&bus), m_AboutDataStore(&store), m_PropertyStore(NULL), m_Listener(&listener)
{
    QCC_DbgTrace(("In ConfigService new Constructor"));
}

ConfigService::ConfigService(ajn::BusAttachment& bus, PropertyStore& store, Listener& listener, const char *path) :
    //BusObject("/Config"), m_BusAttachment(&bus), m_AboutDataStore(NULL), m_PropertyStore(&store), m_Listener(&listener)
    BusObject(path), m_BusAttachment(&bus), m_AboutDataStore(NULL), m_PropertyStore(&store), m_Listener(&listener)
{
...
QStatus ConfigService::Register(const char *ifcName)
{
    ...
    InterfaceDescription* intf = const_cast<InterfaceDescription*>(m_BusAttachment->GetInterface(ifcName));
    if (!intf) {
        //CHECK_RETURN(m_BusAttachment->CreateInterface(CONFIG_INTERFACE_NAME, intf,
        CHECK_RETURN(m_BusAttachment->CreateInterface(ifcName, intf,
...
void ConfigService::UpdateConfigurationsHandler(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    do {
        ...
        for (unsigned int i = 0; i < configMapNumElements; i++) {
            ...
            const int DEVTYPEIP = 0;
            const int DEVTYPEBLUETOOTH = 1;
            const int DEVTYPEZIGBEE = 2;

            member->iface->GetDescriptionSocket(&socketFd);
            member->iface->GetDescriptionClientAddr(&devAddr);    
            member->iface->GetDescriptionDevType(&devType);
            char *strValue;
            qcc::String signature = tempValue->Signature();       
            tempValue->Get(signature.c_str(), &strValue);  
            //member->iface->SendCommandToDev(socketFd, tempKey, strValue, &devAddr);
            if (devType == DEVTYPEIP) {

                    client = (struct sockaddr_in *)&devAddr;
                    if (strcmp(tempKey, "PowerSwitch") == 0) {
                            const int SYSTEMCMD_LEN = 32; 
                            char systemCmd[SYSTEMCMD_LEN];

                            memset(systemCmd, 0, sizeof(systemCmd));
                            sprintf(systemCmd, "tuyaled_cmd %s %s", inet_ntoa(client->sin_addr), (strcmp(strValue, "on") == 0) ? "1":"0");
                            std::cout << "excute system command: " << systemCmd << std::endl;
                            system(systemCmd);
                    }
            
            } else if(devType == DEVTYPEBLUETOOTH) {


            } else if(devType == DEVTYPEZIGBEE) {
                const int BUFLEN = 128;
                char buf[BUFLEN];
                int bytes;

                std::cout << "send to zigbee device" << std::endl;
                memset(buf, 0, BUFLEN);
                sprintf(buf, "key:%s value:%s\r", tempKey, strValue);
                bytes = write(socketFd, buf, strlen(buf)); 
                if (bytes == -1)
                    std::cout << "write error" << std::endl;
                else
                    std::cout << "write " << buf << "buf" << bytes <<" bytes" << std::endl;
                
                memset(buf, 0, BUFLEN);
                bytes = read(socketFd, buf, strlen(buf));
                if (bytes == -1)
                    std::cout << "read error" << std::endl;
                else
                    std::cout << "read " << buf << "buf " <<  bytes  <<" bytes" << std::endl;
